...
class OportunidadeController extends Controller
{
    public function gerenciarOportunidade() 
    {
        ...
        if (Auth::user()->admin) {
            $oportunidades = Oportunidade::select('oportunidades.id','oportunidades.codigo','users.nome as colaborador','clientes.nome as cliente','oportunidades.data_inicio','oportunidades.estagio')
            ...
            $titulos_tabela = array (__('oportunidade.id'),__('oportunidade.codigo'),__('oportunidade.colaborador'),
                                     __('oportunidade.cliente'),__('oportunidade.data_inicio'),__('oportunidade.estagio'));
        } else {
            ...
            if ($cdcFilhos == 0) {
                $oportunidades = Oportunidade::select('oportunidades.id','oportunidades.codigo','clientes.nome as cliente','oportunidades.data_inicio','oportunidades.estagio')
                                            ->join('users', 'users.id', '=', 'oportunidades.id_user')
                                            ->join('clientes', 'clientes.id', '=', 'oportunidades.id_cliente')
                ...
                $titulos_tabela = array (__('oportunidade.id'),__('oportunidade.codigo'),__('oportunidade.cliente'),
                                         __('oportunidade.data_inicio'),__('oportunidade.estagio'));
            } else {
                $oportunidades = Oportunidade::select('oportunidades.id','oportunidades.codigo','users.nome as colaborador','clientes.nome as cliente','oportunidades.data_inicio','oportunidades.estagio')
                                            ->join('users', 'users.id', '=', 'oportunidades.id_user')
                                            ->join('clientes', 'clientes.id', '=', 'oportunidades.id_cliente')
    ...
    public function CadastrarOportunidade()
    {
        ...
        if (Auth::user()->admin) {
             ...
             * Seleciona os CDCs
             */
            $cdcs = ClienteCdc::select('cliente_cdcs.id_cliente','cliente_cdcs.id_cdc','cdcs.cdc','cdcs.nome','cliente_cdcs.area_total','cliente_cdcs.area_irrigada')
            ...
            $clientes = ClienteCdc::select('clientes.id','clientes.nome')
                                  ->join('cdcs', 'cdcs.id', '=', 'cliente_cdcs.id_cdc')
                                  ->join('clientes','clientes.id','=','cliente_cdcs.id_cliente')
                                  ->whereRaw("find_in_set(cdcs.cdc, '".$cdcFilhos."') > 0")
                                  ->distinct()
                                  ->orderBy('clientes.nome')
                                  ->get();
    ...
    public function salvarOportunidade(Request $req)
    {
        ...
        if ($encontrado > 0) {
            ...
            $date = new DateTime($dados['data_inicio'], new DateTimeZone('America/Sao_Paulo'));
    ...
    public function preencheSelecao(Request $req)
    {
        ...
        if ($dependente == 'id_cliente') {
            ...
            $cdcs = "";
            if (Auth::user()->admin) {
                $cdcs = ClienteCdc::select('cliente_cdcs.id_cliente','cliente_cdcs.id_cdc','cdcs.cdc','cdcs.nome','cliente_cdcs.fazenda','cliente_cdcs.area_total','cliente_cdcs.area_irrigada')
                                    ->join('cdcs', 'cdcs.id', '=', 'cliente_cdcs.id_cdc')
                                    ->where('cliente_cdcs.id_cliente',$valor)
                                    ->get();
            } else {
                $cdc_user = Session::get('cdc');
                $cdcFilhos = Session::get('cdcFilhos');
            ...
            foreach ($cdcs as $cdc) {
                $ret .= '<option value="'.$cdc->id_cdc.'">'.$cdc->cdc.' '.$cdc->nome.' ('.$cdc->fazenda.' - '.$cdc->area_total.'ha/'.$cdc->area_irrigada.'ha)</option>';
            }
        }
